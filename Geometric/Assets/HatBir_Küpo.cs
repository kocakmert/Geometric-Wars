...
public class HatBir_Küpo : MonoBehaviour {
    private Hareket ccx_sayac;
    public GameObject küpx, küpy, getir,canbar;
    public Button mybutton;
    public Button kapsülsağ, karesağ, ortasilindir, ortakapsül, ortaküp, solsilindir, solkapsül,silindirsağ;
    int oyuncu = 900;
    ...
    public void Movex()
    {
        küpx.SetActive(true);
        mybutton.gameObject.SetActive(false);
        if (küpx.transform.position == new Vector3(-16.39f, 4.092989f, 0))
        {
    ...
    void Sorgula_Oyuncu()
    {
       // int x = Random.Range(1, 3);
        küpx.transform.position = new Vector2(-16.39f, -8.332f);
        oyuncu = 900;
        küpx.SetActive(false);
        mybutton.gameObject.SetActive(false);
        Invoke("bekle", 1);
